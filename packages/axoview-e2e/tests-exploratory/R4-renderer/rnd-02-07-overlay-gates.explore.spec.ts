...
test.describe('RND-02 — hiding a layer must also hide its connector LABELS', () => {
  test('the connector body disappears but its label chip keeps painting', async ({
    page,
    app
  }) => {
    void app;
    ...
    // Two nodes + a connector between them, built through real input so the
    ...
    const named = await nameConnector(page, LABEL_TEXT);
    expect(named.connectors, 'exactly one connector was drawn').toBe(1);
    expect(named.name).toBe(LABEL_TEXT);
    expect(named.labels, 'the connector carries one renderable label').toBe(1);

    // Clear the selection so no promotion is in play (a SELECTED connector is
    // DOM-promoted and always keeps its labels — that would confound this).
    await page.evaluate(() => {
    ...
    // PRECONDITION 1: with the layer VISIBLE the body paints and the chip shows.
    const seededVisible = await seedLayer(page, true);
    expect(seededVisible.layers).toBe(1);
    expect(seededVisible.visible).toBe(true);
    expect(seededVisible.connectorsOnLayer).toBe(1);
    expect(seededVisible.itemsOnLayer).toBe(2);
    await page.waitForTimeout(900);

    const bodyVisible = await paintedPixels(page, 'axoview-connectors-canvas');
    const chipsVisible = await labelChipCount(page);
    expect(bodyVisible, 'the connector bulk canvas must be painting').toBeGreaterThan(0);
    expect(chipsVisible, 'the label chip must be mounted to start with').toBeGreaterThan(0);

    // Now hide the layer.
    const seededHidden = await seedLayer(page, false);
    expect(seededHidden.visible).toBe(false);
    await page.waitForTimeout(900);

    // PRECONDITION 2: the BODY really did go away — otherwise the layer toggle
    // did nothing and a surviving chip would prove nothing.
    const bodyHidden = await paintedPixels(page, 'axoview-connectors-canvas');
    expect(bodyHidden, 'the connector body must stop painting when its layer is hidden').toBe(0);

    const chipsHidden = await labelChipCount(page);
    // Characterization: the chip survives.
    expect(chipsHidden).toBe(chipsVisible);
  });

  test.fail(
    'BUG: a hidden layer should leave no connector label on the canvas',
    async ({ page, app }) => {
      void app;
      test.setTimeout(180_000);
      const canvas = new CanvasPOM(page);
      ...
      await drawConnector(page, canvas, a, b);
      const named = await nameConnector(page, LABEL_TEXT);
      expect(named.labels, 'PRECONDITION: the connector carries a label').toBe(1);
      await page.evaluate(() => {
        const ui = (window as any).__axoview__.ui.getState();
        ui.actions.setItemControls(null);
        ui.actions.setSelectedIds([]);
      ...
      await page.waitForTimeout(600);
      await seedLayer(page, true);
      await page.waitForTimeout(700);
      expect(
        await labelChipCount(page),
        'PRECONDITION: the chip is mounted while the layer is visible'
      ).toBeGreaterThan(0);

      const hidden = await seedLayer(page, false);
      expect(hidden.visible, 'PRECONDITION: the layer is now hidden').toBe(false);
      await page.waitForTimeout(900);
      expect(
        await paintedPixels(page, 'axoview-connectors-canvas'),
        'PRECONDITION: the body really is hidden'
      ).toBe(0);

      // The assertion under test.
      expect(await labelChipCount(page)).toBe(0);
    }
  );
});
