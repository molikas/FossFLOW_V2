...
describe('Grid background formula — C-1 regression', () => {
  describe('backgroundSize', () => {
    it('at zoom=1, width equals PROJECTED_TILE_SIZE.width', () => {
      const { tileSizeWidth } = computeGridBackground({ scroll: { x: 0, y: 0 }, zoom: 1, elWidth: 800, elHeight: 600 });
      expect(tileSizeWidth).toBeCloseTo(PROJECTED_TILE_SIZE.width, 5);
    });

    it('at zoom=1, height equals PROJECTED_TILE_SIZE.height', () => {
      const { tileSizeHeight } = computeGridBackground({ scroll: { x: 0, y: 0 }, zoom: 1, elWidth: 800, elHeight: 600 });
      expect(tileSizeHeight).toBeCloseTo(PROJECTED_TILE_SIZE.height, 5);
    });

    it('at zoom=2, width doubles', () => {
      const z1 = computeGridBackground({ scroll: { x: 0, y: 0 }, zoom: 1, elWidth: 800, elHeight: 600 });
      const z2 = computeGridBackground({ scroll: { x: 0, y: 0 }, zoom: 2, elWidth: 800, elHeight: 600 });
      expect(z2.tileSizeWidth).toBeCloseTo(z1.tileSizeWidth * 2, 5);
    });

    it('at zoom=0.5, width halves', () => {
      const z1 = computeGridBackground({ scroll: { x: 0, y: 0 }, zoom: 1,   elWidth: 800, elHeight: 600 });
      const zH = computeGridBackground({ scroll: { x: 0, y: 0 }, zoom: 0.5, elWidth: 800, elHeight: 600 });
      expect(zH.tileSizeWidth).toBeCloseTo(z1.tileSizeWidth / 2, 5);
    });

    it('backgroundSize string has correct format', () => {
      const { backgroundSize } = computeGridBackground({ scroll: { x: 0, y: 0 }, zoom: 1, elWidth: 800, elHeight: 600 });
      expect(backgroundSize).toMatch(/^\d+(\.\d+)?px \d+(\.\d+)?px$/);
    });

    it('height in backgroundSize is twice the tile height', () => {
      const { backgroundSize, tileSizeHeight } = computeGridBackground({ scroll: { x: 0, y: 0 }, zoom: 1, elWidth: 800, elHeight: 600 });
  ...
  describe('backgroundPosition', () => {
    it('at scroll={0,0} x-offset equals elWidth/2 + tileWidth/2', () => {
      const elWidth = 800;
      const zoom = 1;
      const { backgroundPosition, tileSizeWidth } = computeGridBackground({ scroll: { x: 0, y: 0 }, zoom, elWidth, elHeight: 600 });
    ...
    it('at scroll={0,0} y-offset equals elHeight/2', () => {
      const elHeight = 600;
      const { backgroundPosition } = computeGridBackground({ scroll: { x: 0, y: 0 }, zoom: 1, elWidth: 800, elHeight });
      const [, yStr] = backgroundPosition.split(' ');
      expect(parseFloat(yStr)).toBeCloseTo(elHeight / 2, 5);
    });

    it('x increases linearly with scroll.x', () => {
      const base = computeGridBackground({ scroll: { x: 0,   y: 0 }, zoom: 1, elWidth: 800, elHeight: 600 });
      const moved = computeGridBackground({ scroll: { x: 100, y: 0 }, zoom: 1, elWidth: 800, elHeight: 600 });
    ...
    it('y increases linearly with scroll.y', () => {
      const base  = computeGridBackground({ scroll: { x: 0, y: 0   }, zoom: 1, elWidth: 800, elHeight: 600 });
      const moved = computeGridBackground({ scroll: { x: 0, y: 150 }, zoom: 1, elWidth: 800, elHeight: 600 });
    ...
    it('negative scroll values produce finite numbers', () => {
      const { backgroundPosition } = computeGridBackground({ scroll: { x: -500, y: -300 }, zoom: 1, elWidth: 800, elHeight: 600 });
    ...
    it('sub-pixel zoom values produce no NaN', () => {
      const { backgroundPosition, backgroundSize } = computeGridBackground({ scroll: { x: 0, y: 0 }, zoom: 0.123, elWidth: 800, elHeight: 600 });
      [...backgroundPosition.split(' '), ...backgroundSize.split(' ')].forEach((s) => {
        expect(isNaN(parseFloat(s))).toBe(false);
      });
    });

    it('is deterministic — same inputs always produce the same output', () => {
      const params: GridBgParams = { scroll: { x: 77, y: -33 }, zoom: 1.5, elWidth: 1024, elHeight: 768 };
      expect(computeGridBackground(params)).toEqual(computeGridBackground(params));
    });
  });
  ...
  describe('first-render behaviour', () => {
    it('formula result is independent of isFirstRender flag (the flag only controls animation duration)', () => {
      ...
      const params: GridBgParams = { scroll: { x: 50, y: 50 }, zoom: 1.2, elWidth: 800, elHeight: 600 };
