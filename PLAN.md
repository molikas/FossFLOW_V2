...
| **QA-EXPLORE** | Exploratory testing program — campaign + remediation + standing agent | `[x]` **SHIPPED 2026-08-08** | ⚠️ Very High | Exploratory remediation shipped (240 campaign bugs, 22 owner rulings, `/explore` agent) — see [ADR 0047](docs/adr/0047-exploratory-testing-program.md) and this file's git history. A hypothesis-and-verification campaign probed **385 falsifiable hypotheses across 27 areas and confirmed 240 bugs while every suite was green** — each finding outside existing coverage by construction of the novelty rule, each carrying a committed expected-fail repro in a CI-quarantined probe lane. Remediation ran as waves 0–6: trust & security, interaction & rendering correctness, consistency & decided UX, then the two design-gated larges — page-stamped history ([ADR 0038](docs/adr/0038-webgl-instanced-render-substrate.md) §8) and **the canvas merge**, which collapsed four bulk WebGL2 contexts into one sorted draw so `zIndex` and `layer.order` finally cross an entity type. Wave 6 closed the picker's matching gap (it now resolves the layer tier *and* cross-type through the renderer's own comparator), froze the record to [docs/reviews/exploratory-2026-07/](docs/reviews/exploratory-2026-07/README.md), and turned the method into the standing [`/explore`](.claude/commands/explore.md) skill — delta-scoped against the last-swept commit, runnable headless. **Durable lesson worth more than the fix list:** the campaign's *evidence* was reliable and its *diagnoses* were hypotheses — a dozen recorded root causes were corrected while being fixed. |
