...
**Status:** Code fixed on branch `fix/shake-out-2026-07-28` (2026-07-28) — the branch now names the signed-in address and offers "Use a different Google account" (`signOut()` clears the profile hint so Google shows the account chooser, then the Drive read is re-attempted; the existing one-shot auto-retry covers only `needs-signin`). **The config gap is still Open:** set `GOOGLE_PROJECT_NUMBER` (and the build-time Google API key) to restore rung 3, otherwise the Picker grant path stays dead and viewers rely on the account switch + owner-side sharing.
