...
- [x] Owner triage of the one new SUSPECT — **A5/CHR-08 ruled 2026-07-30**: optional public base URL in the runtime config with page-origin fallback (option b). Recorded as the 22nd row of [DECISIONS.md](../reviews/exploratory-2026-07/DECISIONS.md); implementation rides Wave 2's share cluster.
...
- [x] **GPU-13 — DONE 2026-08-02.** Design gate closed (`f33624a4`), substrate
  ...
  **Five corrections, all found by implementing** and all recorded in the ADR 0038
  ...
  - **The GRID moved under the bulk.** Its old position (over rectangle fills,
    ...
    between.
  - **`data-all-icons-drawn` was written about the BUILD, not the PAINT** — so
    image export could capture the frame before the icons were on it. Pre-existing
    and marginal (the same test measured 0.001 and 0.042 non-background pixels on
    consecutive runs against a 0.01 floor); the merge's slightly heavier first
    build is what pushed it over. Fixed by writing the attribute after
    `render()`.
...
- [x] Write `.claude/skills/explore.md`: APPROACH distilled + COLDSTART flow + rig-traps appendix + delta-mode area selection (`git diff` vs last campaign end commit); regenerate-baseline step; **embed the end-of-session report contract** (Notes for Claude) so every future run ends with the same four-part report.
  ...
  **Rig-traps appendix — queued contents (grow this list, don't rediscover it):**
  ...
  - **Wave 5: a readiness attribute must be published where its CLAIM becomes true.** `data-all-icons-drawn` gates image export, and it was written inside `buildInstances` — i.e. it claimed "every icon is on screen" one statement before the draw call that put them there. The window was small enough that the export test measured 0.001 and 0.042 non-background pixels on consecutive runs against a 0.01 floor, which reads as flake rather than as a defect. When a signal is consumed as "it is safe to capture now", ask which line makes it true and publish it there.
