...
> **Already built for all four packages (2026-07-30, S1–S3) — do not rebuild, and do not touch these configs again.** The S/A tracks and F5 live in `axoview-app`, S2 in `axoview-backend`/`axoview-worker`, so each got the same three-line config plus a root script: `explore:unit` (lib) · `explore:unit:app` · `explore:unit:backend` · `explore:unit:worker` · `explore:e2e`. The app config needed `'/__explore__/'` in its `testPathIgnorePatterns` (it has no `testMatch`, so Jest's defaults would sweep a probe in); **backend and worker needed zero touch** — both already scope `testMatch` to `__tests__/**`. Quarantine re-verified at each step: lib 155 suites/1737, app 26/268, backend 7/102, worker 4/124, all green and unchanged.
...
- **Opening and closing the Export-as-image dialog DESTROYS `window.__axoview__`.** `Axoview`'s debug-bridge effect ends with `delete win.__axoview__`, and the dialog mounts a SECOND Axoview (the hidden export instance) — so its unmount deletes the *main* app's bridge and every later `page.evaluate` through the bridge throws `Cannot read properties of undefined`. Worse, **while the dialog is open the bridge points at the hidden export instance**, not the app, and `document.querySelector('[data-testid="axoview-nodes-canvas"]')` may match its canvas first. Do all bridge work before the first open (R3/GPU-02), or drive state through `page.route` instead.

Known setup-throw traps added by S1–S3:

- **`axoview-backend` is native ESM, so the `jest` global is NOT injected.** `jest.setTimeout(...)` throws `ReferenceError: jest is not defined` and takes the whole suite with it. Pass a per-test timeout as `test()`'s third argument instead (or `import { jest } from '@jest/globals'`).
- **Mocking a module with `jest.mock` drops the classes the component `instanceof`-checks.** `DriveShareManageDialog`'s error copy does `err instanceof DriveShareError`; a factory that omits it throws `Right-hand side of 'instanceof' is not an object` **only on the failure path** — which is exactly the path the probe is testing, so it reads as evidence. Re-export the real class: `DriveShareError: jest.requireActual('…/driveSharing').DriveShareError`.
- **`jest.spyOn` returns the EXISTING spy on a second call**, so call counts accumulate across tests in one file. `mockClear()` right after acquiring it, or a precondition assertion silently reads the previous test's calls (this cost one wrong AUTH-10 precondition).
- **MUI Autocomplete renders more than one `role="combobox"`** — `getByRole('combobox')` throws "found multiple elements". Target `[data-axoview-id="…"]` instead.
...
- **`[data-label-hit-id]` is published by TWO layers** — `LabelHitLayer` (floating Labels) and `NodeLabelHitLayer` (node names). Counting the bare attribute conflates them; node names are `[data-axoview-id="canvas-label-hit"]` specifically (R3/GPU-05). Same trap shape as the `<p>` that matches both the on-canvas label and the sidebar's copy of the name — scope DOM queries to the renderer subtree (`[data-testid="axoview-canvas"]`) when you mean "on the canvas".
- **A pixel-band oracle needs a per-column profile, not a total.** R3/GPU-09 measured "did the glyphs overrun the chip's right padding?" over the whole padding band and its SHORT-name control failed: a name that exactly fits ends *at* the band's outer edge, so antialiasing tints the first columns. Return the per-column counts and assert on the inner half. (The control failing is the rig working — a band that can't distinguish fits from overruns would have "confirmed" the bug either way.)
...
- **App-package T1 harness (S1–S3, reusable by every A area and F5).** `packages/axoview-app/src/__explore__/S1/harness.ts` — `resetAuth()` (which also drains the **module-level** `pendingAuthTimeout`, not just store state), `installBridge()`, `seedNearExpirySession()` / `seedHealthySession()`, `waiterCount()`, and **`settle()`: a timer-free "did this promise settle?" oracle**. That last one exists because the auth store can leave a promise that *nothing* will ever settle (AUTH-01) — a state no timeout-based check can distinguish from "slow". Component probes follow `components/__tests__/DriveAccessRequiredDialog.test.tsx`: mock `react-i18next` to the English fallback (interpolate `{{vars}}` yourself when a component passes a `{ defaultValue }` object) and stub `AppStorageContext` / `DiagramLifecycleProvider`. Rendering the real component is what turned several store-level findings into user-visible ones (AUTH-05, DRV-08).
- **Real-Express T2 tier (S2).** `packages/axoview-backend/src/__explore__/S2/share-08-09-10-14.explore.spec.js` spawns the actual `server.js` as a child process against a temp `STORAGE_PATH` and speaks HTTP to it (readiness polled on `/healthz`; `node:http` where a header browsers forbid — `Host` — must be set). `server.js` calls `app.listen()` at import and exports nothing, so a child process is the only way to reach the middleware stack without touching product code. Copy this whenever a claim is about CORS, body parsing, error middleware or a route-table flag rather than a handler.
- **Backend/worker handler tier.** `routes.js` handlers are pure over an adapter — reuse `src/__tests__/helpers/memoryAdapter.js` (`createMemoryAdapter`, `decodeJson`, `putJson`, `makeCtx`). **But that double diverges from production:** it keeps `diagrams/folders` and `folders` in separate map slots and carries its own reserved-key filter, so it *hides* the SHARE-02 collision entirely. Any claim about key layout must use the real fs adapter against a `mkdtemp` dir. The worker is driven through Hono's `app.request(path, init, env)` with `fetch` stubbed — no wrangler.
...
- Run probes with `npm run explore:unit` (lib) / `explore:unit:app` / `explore:unit:backend` / `explore:unit:worker` / `npm run explore:e2e`. **Pick the script for the package the code under test lives in** — `explore:unit` is rooted in `axoview-lib` and will silently find nothing for an app/backend/worker probe. Single file: `npm run explore:unit:app -- --testPathPattern "<slug>"`, or `npx playwright test --config packages/axoview-e2e/playwright.explore.config.ts tests-exploratory/<area>/<file>` from the repo root. `-g "<ID>"` re-runs one hypothesis.
- **File naming differs per package** (each config's `testMatch` is exact): lib + app want `**/__explore__/**/*.explore.test.{ts,tsx}`; backend wants `src/**/__explore__/**/*.explore.spec.js`; worker wants `src/**/__explore__/**/*.explore.spec.ts`. A misnamed file is not an error — it is simply never run.
...
- **Writing verdicts into a markdown table with `String.replace` is a trap — two bit in S1.** (a) A `$1` backreference inside a bash double-quoted `node -e` string prepended every replacement to the **top of the file** instead of into the row. (b) A `||` anywhere in the evidence text (e.g. `data.email || ''`) splits the row into extra cells and silently corrupts the table. Use a line-oriented script file: match on the `| <ID> |` prefix, assert the line ends with `| PROPOSED | |`, slice that off and concatenate — no regex replacement string — and escape `|` as `&#124;`. Then verify by counting cells per row (`line.split('|').length` must be constant) before moving on.
- **Two agents can share this working tree.** Before editing `LEDGER.md`, `known_issues.md` or the root `package.json`, run `git status` and `git diff` on them: another wave may hold uncommitted edits there (a concurrent F-track session marked its own row `IN PROGRESS` mid-S3). Prefer surgical single-line edits over read-modify-write scripts on those three files, append `known_issues.md` entries at the tail only, and commit just the files your wave owns.
...
**From the share/backend block (S1–S3):**

S-a. **The exit ramps are one function written several times, and each forgets a different part of the ritual.** `signOut` / `markExpired` / `markDriveScopeMissing` / `_onToken`'s scope-less hard stop all park the auth session, and between them they drop waiter draining, timeout clearing, `_absorbStaleError` and the profile hint in different combinations — nine of S1's twelve bugs. Same shape one layer down: `deleteDiagram` cascades to the public snapshot, the soft delete the UI actually performs does not (SHARE-06). Whenever an area has several ways to reach one state, diff the rituals rather than testing one.

S-b. **Nothing serialises a read-modify-write.** `folders.json` (SHARE-03) and `shareDiagram` (SHARE-04) both read, mutate and write back with no lock or version; the fs adapter's tmp+rename is *file* atomicity, which is the wrong granularity. Every existing suite is sequential, so `await Promise.all([op(), op()])` is a one-line probe no area had run.

S-c. **The wiring around a well-tested handler is where the handler's contract dies.** `routes.js` has 111 green tests and returns `{ error }` JSON throughout; the Express app mounts no error middleware, so body-parser failures return HTML with a stack trace (SHARE-08), and CORS withholds the *response* without blocking the *request* (SHARE-09). Test the middleware stack, not just the handler.

S-d. **A typed failure is only as good as what the caller does with it.** Nearly every S3 bug is caller-side: the ladder classifies four terminal causes and the gate renders one message (DRV-02), the Picker distinguishes wrong-file from cancel and the gate does not (DRV-03), `afterGrant` decides recoverable-vs-terminal and nothing ever clears it (DRV-01). Service-level suites all stop at the return value — follow it to its consumer.

S-e. **Enum coverage stops at the values the happy path produces.** `type:'domain'` is declared and unhandled (DRV-04); an absent `meta.size` reads as zero and disables a cap (SHARE-07); a proxy 400 is unclassified (DRV-12). For any union an area touches, enumerate the declared values and check each has a branch.

S-f. **Two harvested "invariants" in coverage-baseline.md were STALE.** The worker's resourceKey allowlist is `{1,120}`, not the recorded `{10,120}` (DRV-11), and `getFileShareMeta` — the only reader of `resourceKey` — has no caller at all (DRV-15). Verify a harvested invariant against the source before building a probe on it, and correct the baseline when it is wrong.
